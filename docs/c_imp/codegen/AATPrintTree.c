...
void printAAT(AATstatement body) {
  ...
  case AAT_CONDITIONALJUMP:
    ...
    AATindent--;
    break;
  case AAT_CALLSTATEMENT:
    printAATindent(AATindent);
    printf("CALL STATEMENT: %s\n",body->u.callStatement.jump);
    AATindent++;
    printExpList(body->u.callStatement.actuals);
    AATindent--;
    break;
  case AAT_SEQ:
...
void printAATexp(AATexpression exp) {
  ...
  case AAT_OPERATOR:
    ...
    break;
  case AAT_CALLEXPRESSION:
    printAATindent(AATindent);
    printf("CALL EXPRESSION: %s\n", exp->u.callExpression.jump);
    AATindent++;
    printExpList(exp->u.callExpression.actuals);
    AATindent--;
    break;
  case AAT_CONSTANT:
    printAATindent(AATindent);
